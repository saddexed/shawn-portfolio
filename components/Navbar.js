...
export default function Navbar() {
  ...
  const pages = ['About', 'Projects', 'Resume', 'Languages', 'Setup']; // 'Article' removed for now
  ...
  return (
    <header className="text-primary absolute top-0 z-3 mt-3.25 flex min-h-14.75 w-full flex-wrap items-center text-xs lg:mt-0">
      <Link
        href="/"
        className="hover:bg-hover font-display text-primary ml-3 order-1 flex h-8.5 cursor-pointer appearance-none items-center rounded-lg border-none bg-transparent px-2.5 text-[2rem] leading-none font-bold no-underline transition-all duration-300 ease-in-out"
      >
        <span>se</span>
      </Link>
      <nav className="w-full lg:w-auto order-3 lg:order-2 lg:flex-1 text-center max-lg:overflow-x-auto max-lg:overflow-y-hidden max-lg:[scrollbar-width:none] max-lg:[&::-webkit-scrollbar]:hidden">
      ...
      <div className="mr-3 ml-auto order-2 lg:order-3 flex items-center">
        <button
          type="button"
          aria-label="Command"
          onClick={toggle}
